...
<%  } %>    
    <form name="epersongroup" method="post" action="">    
            ...
            <%-- <input type="submit" name="submit_add" value="Add EPerson..."> --%>
            	<input class="btn btn-secundary col-md-2 corlogoGlobal fcorbrancoGlobal" type="submit" name="submit_add" value="<fmt:message key="jsp.dspace-admin.eperson-main.add"/>" />
			</div>
			<br/>
			...
			<div class="col-md-12 espGlobal">
						<fmt:message key="jsp.dspace-admin.eperson-main.then"/>
			</div>
			<div class="col-md-12">
			<input type="submit" class="btn btn-default col-md-2 corlogoGlobal fcorbrancoGlobal" name="submit_edit" value="<fmt:message key="jsp.dspace-admin.general.edit"/>" onclick="javascript:finishEPerson();"/>
						
            <% if(loginAs) { %>&nbsp;<input type="submit" class="btn btn-default col-md-2" name="submit_login_as" value="<fmt:message key="jsp.dspace-admin.eperson-main.LoginAs.submit"/>" onclick="javascript:finishEPerson();"/> <% } %>
            
            <%-- <input type="submit" name="submit_delete" value="Delete..." onclick="javascript:finishEPerson();"> --%>
            <input type="submit" class="btn btn-danger espGlobalesq col-md-2" name="submit_delete" value="<fmt:message key="jsp.dspace-admin.general.delete-w-confirm"/>" onclick="javascript:finishEPerson();"/>
