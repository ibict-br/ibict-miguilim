...
<%
    ...
    //SortedMap<String, String> paginasDeRevisao = (SortedMap<String, String>) reviewJSPs;

    //get an iterator to loop through the review JSPs to load
    Iterator reviewIterator = reviewJSPs.keySet().iterator();
    Iterator reviewIterator2 = reviewJSPs.keySet().iterator();
%>
<dspace:layout locbar="off" navbar="off" titlekey="jsp.submit.review.title" style="submission" nocache="true">
    <form action="<%= request.getContextPath() %>/submit" method="post" onkeydown="return disableEnterKey(event);">
        ...
        <h1><fmt:message key="jsp.submit.review.heading"/>
            <dspace:popup
                    page="<%= LocaleSupport.getLocalizedMessage(pageContext, \"help.index\") + \"#verify\"%>"><fmt:message
                    key="jsp.morehelp"/></dspace:popup>
        </h1>
        <p><fmt:message key="jsp.submit.review.info1"/></p>
        <div class="alert alert-info-customizado"><fmt:message key="jsp.submit.review.info2"/></div>
        <p><fmt:message key="jsp.submit.review.info3"/></p>
        <p><fmt:message key="jsp.submit.review.info4"/></p>
        <div class="container">
            <%
                // determina o valor máximo de paginas
                int maxPageNum = 0;
                while (reviewIterator2.hasNext()) {
                    String stepAndPage = (String) reviewIterator2.next();
                ...
                while (reviewIterator.hasNext()) {
                    ...
                    reviewJSP = (String) reviewJSPs.get(stepAndPage);
            %>
            <div class="well row">
                    <%--Load the review JSP and pass it step & page info--%>
                <jsp:include page="<%=reviewJSP%>">
                    <jsp:param name="submission.jump" value="<%=stepAndPage%>"/>
                    <jsp:param name="maxPageNum" value="<%=maxPageNum%>"/>
                </jsp:include>
            </div>
            <%
                    //out.write("pagina incluida: " + reviewJSP + " stepAndPage:" + stepAndPage + "<br/>");
                }
            %>
        </div>
        ...
        <div class="col-md-6 pull-right btn-group">
            <input class="btn btn-secondary col-md-4 fcorbrancoGlobal" type="submit" name="<%=AbstractProcessingStep.PREVIOUS_BUTTON%>"
                   value="<fmt:message key="jsp.submit.review.button.previous"/>"/>
            <input class="btn btn-secondary col-md-4 corLaranjaGlobal corFontelogoGlobal" type="submit" name="<%=AbstractProcessingStep.CANCEL_BUTTON%>"
                   value="<fmt:message key="jsp.submit.review.button.cancelsave"/>"/>
            <input class="btn btn-success col-md-4 corlogoGlobal" type="submit" name="<%=AbstractProcessingStep.NEXT_BUTTON%>"
                   value="<fmt:message key="jsp.submit.review.button.deposit.completed"/>"/>
        </div>
    </form>
</dspace:layout>
