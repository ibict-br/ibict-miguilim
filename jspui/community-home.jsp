...
<div class="well">
<div class="row">
	<div class="col-md-8 espGlobalesq15Invert">
    <!-- Titulo Miguilim -->
        <h1 class="espGlobal"><%= name %>
        <%
...
%>
		<!-- Miguilim - Página inicial da comunidade -->
    <h3 class="espGlobal"><fmt:message key="jsp.community-home.heading1"/></h3>
...
<%
	...
	{ %>
    <!-- Título: Novos registros no Miguilim -->
	<h1 class="espGlobalesq15"><fmt:message key="browse.page-title.carousel"/></h1>
	<div class="col-md-12">
	...
	<%	for (int i = 0; i < items.size(); i++)
		{
			...
			%>
		    <div style="padding-top:20px; min-height: 80px;" class="item <%= first?"active":""%>">
		      <div class="carousel-fonte" style="padding-left: 80px; padding-right: 80px; display: inline-block;"> 
			  <p>Título: <a href="<%= request.getContextPath() %>/handle/<%=items.get(i).getHandle() %>"><%= Utils.addEntities(StringUtils.abbreviate(displayTitle, 400)) %></a></p>
		      </div>
		    </div>
<%
				first = false;
...
<div class="panel panel-primary">
	<div class="panel-heading-customizado"><fmt:message key="jsp.general.browse"/></div>
...
%>
	<form method="get" action="<%= request.getContextPath() %>/handle/<%= community.getHandle() %>/browse">
		...
		<!--<input class="btn btn-default col-md-4" type="submit" name="submit_browse" value="<fmt:message key="<%= key %>"/>"/>-->
		<input class="css3button col-md-4" type="submit" name="submit_browse" value="<fmt:message key="<%= key %>"/>"/>
	</form>
<%	
	}
%>
...
%>
	<div class="col-md-6">
		...
		<h3 class="fontGlobal"><fmt:message key="jsp.community-home.heading3"/></h3>
...
<% }  %>		
	      ...
	      <h4 class="list-group-item-heading"><a class="fontGlobal" href="<%= request.getContextPath() %>/handle/<%= subcommunities.get(j).getHandle() %>">
...
%>
	    		<% if (remove_button) { %>
	                <form class="btn-group" method="post" action="<%=request.getContextPath()%>/tools/edit-communities">
	                  ...
	                  <button type="submit" class="btn btn-xs btn-secondary corTrans"><span class="glyphicon glyphicon-trash"></span></button>
	                </form>
	    		<% } %>
			    </h4>
...
%>
	<div class="col-md-6">
		...
		<h3 class="fontGlobal espGlobalesq10"><fmt:message key="jsp.community-home.heading2"/></h3>
		<div class="list-group">
<%
...
<% }  %>		
	      ...
	      <h4 class="list-group-item-heading"><a class="fontGlobal" href="<%= request.getContextPath() %>/handle/<%= collections.get(i).getHandle() %>">
		 ...
		 <div class="panel panel-warning">
             ...
             <% if(editor_button) { %>
	            <form method="post" action="<%=request.getContextPath()%>/tools/edit-communities">
                  ...
                  <input class="css3button col-md-12" type="submit" value="<fmt:message key="jsp.general.edit.button"/>" />
             ...
             <% if(add_button) { %>
				...
				<form method="post" action="<%=request.getContextPath()%>/tools/collection-wizard">
		     		<input type="hidden" name="community_id" value="<%= community.getID() %>" />
                    <input class="css3button col-md-12" type="submit" value="<fmt:message key="jsp.community-home.create1.button"/>" />
                ...
                <form method="post" action="<%=request.getContextPath()%>/tools/edit-communities">
                    ...
                    <input class="css3button col-md-12" type="submit" name="submit" value="<fmt:message key="jsp.community-home.create2.button"/>" />
            ...
            <% if( editor_button ) { %>
                <form method="post" action="<%=request.getContextPath()%>/mydspace">
                  ...
                  <input class="css3button col-md-12" type="submit" value="<fmt:message key="jsp.mydspace.request.export.community"/>" />
              ...
              <form method="post" action="<%=request.getContextPath()%>/mydspace">
                ...
                <input class="css3button col-md-12" type="submit" value="<fmt:message key="jsp.mydspace.request.export.migratecommunity"/>" />
               ...
               <form method="post" action="<%=request.getContextPath()%>/dspace-admin/metadataexport">
                 ...
                 <input class="css3button col-md-12" type="submit" value="<fmt:message key="jsp.general.metadataexport.button"/>" />
