...
<dspace:layout locbar="commLink" title="<%= name %>" feedData="<%= feedData %>">
    ...
    <!-- Titulo: Portais e Periodicos -->
    <h1 class="espGlobalesq15Invert"><%= name %>
<%
...
%>
		<h3 class="fontGlobal2 espGlobal espGlobalesq15Invert"><fmt:message key="jsp.collection-home.heading1"/></h3>
      <a class="statisticsLink btn btn-info espGlobalesq15Invert" href="<%= request.getContextPath() %>/handle/<%= collection.getHandle() %>/statistics"><fmt:message key="jsp.collection-home.display-statistics"/></a>
      </h1></div>
<%  if (logo != null) { %>
        <div class="col-md-4">
...
<% 	} %>
  ...
  <!--<div class="panel panel-primary">-->
  	<!--<div class="panel-heading-customizado">
...
%>
	<!--<form method="get" class="btn-group" action="<%= request.getContextPath() %>/handle/<%= collection.getHandle() %>/browse">
		...
		<!--<input type="submit" class="css3button" name="submit_browse" value="<fmt:message key="<%= key %>"/>"/>
	</form>-->
<%	
	}
%>	<!--</div>-->
...
<%  if (submit_button)
    { %>
          <form class="form-group" action="<%= request.getContextPath() %>/submit" method="post">
			...
			<input class="btn btn-secondary col-md-12" type="submit" name="submit" value="<fmt:message key="jsp.collection-home.submit.button"/>" />
          </form>
<%  } %>
        <form class="well espGlobalesq15Invert" method="get" action="">
...
<%  } else { %>
 ...
 -->
				...
				<input class="btn-info-customizado" type="submit" name="submit_subscribe" value="<fmt:message key="jsp.collection-home.subscribe"/>" />
<%  }
    if(feedEnabled)
    { %>
...
<% if( admin_button ) { %>
                 <form method="post" action="<%=request.getContextPath()%>/tools/itemmap">
				  ...
				  <input class="css3button col-md-12" type="submit" value="<fmt:message key="jsp.collection-home.item.button"/>"				  
                </form>
<% if(submitters != null) { %>
		      <form method="get" action="<%=request.getContextPath()%>/tools/group-edit">
				...
				<input class="css3button col-md-12" type="submit" name="submit_edit" value="<fmt:message key="jsp.collection-home.editsub.button"/>" />
		      </form>
<% } %>
<% if( editor_button || admin_button) { %>
                <form method="post" action="<%=request.getContextPath()%>/mydspace">
				  ...
				  <input class="css3button col-md-12" type="submit" value="<fmt:message key="jsp.mydspace.request.export.collection"/>" />
                </form>
               <form method="post" action="<%=request.getContextPath()%>/mydspace">
                 <input type="hidden" name="collection_id" value="<%= collection.getID() %>" />
				 ...
				 <input class="css3button col-md-12" type="submit" value="<fmt:message key="jsp.mydspace.request.export.migratecollection"/>" />
               </form>
