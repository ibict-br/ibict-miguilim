...
%>
		...
		$('#tlocation').children('option[value="123456789/1"]').show();
		});
		</script>
...
<% if (appliedFilters.size() > 0 ) { %>                                
		...
		<%
			...
			for (String[] filter : appliedFilters)
			{
				...
				<input type="text" id="filter_value_<%=idx %>" name="filter_value_<%=idx %>" value="<%= Utils.addEntities(filter[2]) %>" size="45" disabled/>
