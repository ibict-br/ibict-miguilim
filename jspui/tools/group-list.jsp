...
<dspace:layout style="submission" titlekey="jsp.tools.group-list.title"
	...
	<p class="alert alert-info fontdefaultsize corlogoGlobal fcorbrancoGlobal espGlobalesq1Invert"><fmt:message key="jsp.tools.group-list.note1"/></p>	
	<p class="alert alert-warning fontdefaultsize corlogoGlobal fcorbrancoGlobal"><fmt:message key="jsp.tools.group-list.note2"/></p>
   	
    <form method="post" action="">
        <% if(isAdmin){ %>
            <div class="row col-md-offset-5">
                    <input class="btn btn-secondary corlogoGlobal" type="submit" name="submit_add" value="<fmt:message key="jsp.tools.group-list.create.button"/>" />
            </div>
        <% } %>
    </form>
	<br/>
...
%>                  
  		        ...
  		        <input class="btn btn-secondary col-md-6 corlogoGlobal" type="submit" name="submit_edit" value="<fmt:message key="jsp.tools.general.edit"/>" />
                   </form>
<%
	}
...
%>   
                    <form method="post" action="">
                        <input type="hidden" name="group_id" value="<%= groups.get(i).getID() %>"/>
	                <input class="btn btn-secondary col-md-6 corVermelhoGlobal" type="submit" name="submit_group_delete" value="<fmt:message key="jsp.tools.general.delete"/>" />
<%
	}
%>	                
                    </form>
