...
%>
        ...
        <!-- Menu para nova página do Termômetro -->

<!--
        <div class="row">
            <div class="col-md-12  link-customizado-menu">
                <nav class="navbar navbar-default" id="menu" role="navigation">
                        <a class="btn-menu-1" href="<%=locationLink %>?mode=simple">Registro do Item</a>
                        <a class="btn-menu-2" href="<%= request.getContextPath() %>/handle/<%= handle %>/statistics">Metricas</a>
                        <a class="btn-menu-3"  href="#header-termometro">Termômetro</a>
                        --<a class="btn-menu-3"  href="<%= request.getContextPath() %>/funcionamento-termometro.jsp">Termômetro</a>--               
                </nav>
            </div>
        </div>
-->
...
<div id="tabs">
  <ul>
    <li><a class="btn-menu-1" href="#lista-customizada">Registro do item</a></li>
    <li><a class="btn-menu-2" href="#metricas">Estatísticas de uso</a></li>
    <li><a id ="ocultarTermometro" class="btn-menu-3" href="#conteudo-termometro">Termômetro</a></li>
    <li><a id ="ocultarSolicitacao" class="btn-menu-4" href="#solicitacao">Solicitar edição da revista</a></li>
    <li><a id="parametroEmailIssn" class="btn-menu-5">Editar dados do registro</a></li>
  </ul>

  <div id="lista-customizada">
  </div>

 
 <div id="metricas">
<iframe width="650" height="1600" frameborder="0" scrolling="no" marginheight="0" marginwidth="0"
        src="<%= request.getContextPath() %>/handle/<%= handle %>/statistics">
    </iframe>
 </div>
        ...
        <!-- Fim Menu para nova página do Termômetro -->
        ...
        <dspace:sidebar>
            <div class="panel panel-warning">
                ...
                <form method="get" action="<%= request.getContextPath() %>/tools/edit-item">
                    ...
                    <input class="css3button col-md-12" type="submit" name="submit" value="<fmt:message key="jsp.general.edit.button"/>" />
                ...
                <form method="post" action="<%= request.getContextPath() %>/mydspace">
                    ...
                    <input class="css3button col-md-12" type="submit" name="submit" value="<fmt:message key="jsp.mydspace.request.export.item"/>" />
                ...
                <form method="post" action="<%= request.getContextPath() %>/mydspace">
					...
					<input class="css3button col-md-12" type="submit" name="submit" value="<fmt:message key="jsp.mydspace.request.export.migrateitem"/>" />
                ...
                <form method="post" action="<%= request.getContextPath() %>/dspace-admin/metadataexport">
					...
					<input class="css3button col-md-12" type="submit" name="submit" value="<fmt:message key="jsp.general.metadataexport.button"/>" />
                	...
                	<form method="get" action="<%= request.getContextPath() %>/tools/version">
                    	...
                    	<input class="css3button col-md-12" type="submit" name="submit" value="<fmt:message key="jsp.general.version.button"/>" />
                	...
                	<form method="get" action="<%= request.getContextPath() %>/tools/history">
                    	...
                    	<input class="css3button col-md-12" type="submit" name="submit" value="<fmt:message key="jsp.general.version.history.button"/>" />
                	</form>         	         	
					<% } %>
             </div>
          </div>
...
<%
    String locationLink2 = request.getContextPath() + "/handle/" + handle;
...
%>
    <form class="col-md-4" method="post" action="<%= request.getContextPath() %>/view-workspaceitem">
        ...
        <input class="btn btn-primary corlogoGlobal" type="submit" name="submit_simple" value="<fmt:message key="jsp.display-item.text1"/>" />
    </form>
<%
        }
        else
        {
%>
    <a class="btn btn-primary corlogoGlobal" href="<%=locationLink2 %>?mode=simple">
...
%>
    <form class="col-md-4" method="post" action="<%= request.getContextPath() %>/view-workspaceitem">
        <input type="hidden" name="workspace_id" value="<%= workspace_id.intValue() %>" />
        <input class="btn btn btn-primary corlogoGlobal" type="submit" name="submit_full" value="<fmt:message key="jsp.display-item.text2"/>" />
    </form>
<%
        }
        else
        {
%>
    <!--<a class="btn btn-primary" href="<%=locationLink2 %>?mode=full">
...
<%
    ...
    {
%>
   <form class="col-md-4" method="post" action="<%= request.getContextPath() %>/workspace">
        <input type="hidden" name="workspace_id" value="<%= workspace_id.intValue() %>"/>
        <input class="btn btn-primary corlogoGlobal" type="submit" name="submit_open" value="<fmt:message key="jsp.display-item.back_to_workspace"/>"/>
    </form>
<%
    } else {
...
%>
    <!--<a class="statisticsLink  btn btn-primary margem-estatistica" href="<%= request.getContextPath() %>/handle/<%= handle %>/statistics"><fmt:message key="jsp.display-item.display-statistics"/></a>-->
		...
		<tr>			
			...
			<td headers="tt2" class="oddRowOddCol"><a href="<%= url %>"><%= identifier %></a><%= item.getID()==versRow.getItem().getID()?"<span class=\"glyphicon glyphicon-asterisk\"></span>":""%></td>
                        ...
                        <% if(admin_button) { %> 
                            ...
                            <td headers="tt3" class="oddRowEvenCol">
							<a href="mailto:<%= versRowPerson.getEmail() %>"><%=versRowPerson.getFullName() %></a> </td>
                        <% } 
...
%>
    ...
    <!--<p class="submitFormHelp alert alert-info-copyright"><fmt:message key="jsp.display-item.text3"/> <a href="<%= cc_url %>"><fmt:message key="jsp.display-item.license"/></a>
...
%>
    ...
    <!--<p class="submitFormHelp alert alert-info-copyright"><fmt:message key="jsp.display-item.copyright"/></p>-->
...
%>
    ...
    <div id="conteudo-termometro">
        <%@include file="./termometro.jsp"%>
    </div>
</div>
...
<script type="text/javascript" src="<%= request.getContextPath() %>/static/js/tooltip.js"></script>
<script type="text/javascript" src="<%= request.getContextPath() %>/static/js/modificandoDisplay.js"></script>
<!--<script type="text/javascript" src="<%= request.getContextPath() %>/static/js/modificandoDisplayVersao2.js"></script>-->
